...
if __name__ == "__main__":
    ...
    parser.add_argument("--lora_parts", default="emb,time,ln,gate,att_r,att_k,att_v,ffn_r,ffn_k,ffn_v", type=str)
...
"""
    ...
    if Dummy == 1:
        if args.lora:
            ...
            enable_emb_finetune = 'emb'  in LORA_CONFIG["parts"]
            enable_output_finetune = 'output'  in LORA_CONFIG["parts"]
            enable_head_finetune = 'head'  in LORA_CONFIG["parts"]
        model = RWKV(args)
                ...
                if any(n.startswith("lora_") for n, _ in module.named_parameters()):
                    ...
                    for pname, param in module.named_parameters():
                        if pname.startswith("time"):
                            ...
                            param.requires_grad = True
                elif enable_emb_finetune and '.emb' in name:
                    print(f'  LoRA additionally training module {name}')
                    for param in module.parameters():
                        param.requires_grad = True
